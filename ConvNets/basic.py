...
with graph.as_default():
    ...
    tf_train_labels = tf.placeholder(dtype=tf.float32, shape= (batch_size, num_classes))
    ...
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.03).minimize(regularized_loss)
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps) :
        ...
        if(step % 50==0):
            print("Minibatch Loss at step %d is %f" %(step, l))
            print("Minibatch Train Accuracy : %f %%" % accuracy(prediction, batch_labels))
            print("Minibatch Valid Accuracy : %f %%" % accuracy(valid_prediction.eval(), valid_labels))
